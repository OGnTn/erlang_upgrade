...
#
# When runnnig this script, make sure the virtual environment is activated!
# $ source venv/bin/activate
...
BENCHMARK_NAME = "message-instances"
...
def calculate_speedups(results: dict[int, list[float]]) -> dict[int, list[float]]:
    ...
    speedups = {i: [base / t for t in result]
...
def plot_speedup_boxplots(speedups: dict[int, list[float]]):
    ...
    ax.set_xticklabels(
        [k if k % 4 == 0 or k == 1 else "" for k in speedups.keys()])
...
def plot_speedup_errorbars(speedups: dict[int, list[float]]):
    ...
    errors_down = [np.median(speedups[i]) -
                   np.quantile(speedups[i], 0.25) for i in speedups]
    errors_up = [np.quantile(speedups[i], 0.75) -
                 np.median(speedups[i]) for i in speedups]
